...
import { Heart, Search, UserPlus, UserCheck, Clock, Filter, ChevronDown, CheckCircle, X } from 'lucide-react';
...
export default function ConnectionsPage() {
  ...
  const handleAcceptInvite = async (connectionId) => {
    ...
    try {
      ...
      setReceivedInvites(prev => prev.filter(invite => invite.connectionId !== connectionId));
  ...
  const filterItems = (items) => {
    ...
    // Sort by date
    filtered.sort((a, b) => {
      const dateA = new Date(sortBy === 'newest' ? a.respondedAt || a.requestedAt : a.respondedAt || a.requestedAt);
      const dateB = new Date(sortBy === 'newest' ? b.respondedAt || b.requestedAt : a.respondedAt || a.requestedAt);
            ...
            <div className="flex flex-col sm:flex-row border-b">
              <button
                className={`flex-1 py-5 font-medium border-b-2 transition-colors duration-200 ${
                  activeTab === 'connections' ? 'text-rose-600 border-rose-600' : 'text-gray-500 border-transparent hover:text-rose-500'
                }`}
              ...
              <button
                className={`flex-1 py-5 font-medium border-b-2 transition-colors duration-200 ${
                  activeTab === 'received-invites' ? 'text-rose-600 border-rose-600' : 'text-gray-500 border-transparent hover:text-rose-500'
                }`}
            ...
            <div className="p-6">
              {isLoading ? (
                ...
                <>
                    ...
                    <>
                      {getPaginatedItems(connections).length > 0 ? (
                              ...
                              <div className="flex flex-col sm:flex-row items-center">
                                <div className="flex-shrink-0 mb-4 sm:mb-0">
                                  <div className="w-24 h-24 rounded-lg overflow-hidden border-2 border-rose-100 shadow-md">
                                    <img 
                                      src={connection.profileImageUrl ? `${BASE_IMAGE_URL}${connection.profileImageUrl}` : '/default-avatar.jpg'} 
                                ...
                                <div className="sm:ml-6 flex-1 text-center sm:text-left">
                                  <h3 className="font-semibold text-lg text-gray-900">{connection.username}</h3>
                                  <p className="text-sm text-gray-500 mt-1">Connected since: {formatDate(connection.respondedAt)}</p>
                                </div>
                        ...
                        <div className="text-center py-16">
                          ...
                          <h3 className="text-xl font-medium text-gray-900">No connections yet</h3>
                          <p className="text-gray-500 mt-2">Start by sending connection requests to other users</p>
                        </div>
                      )}
                    </>
                  )}
                    ...
                    <>
                      {getPaginatedItems(sentInvites).length > 0 ? (
                            ...
                            >
                              <div className="flex flex-col sm:flex-row items-center">
                                <div className="flex-shrink-0 mb-4 sm:mb-0">
                                  <div className="w-24 h-24 rounded-lg overflow-hidden border-2 border-rose-100 shadow-md">
                                    <img 
                                      src={invite.profileImageUrl ? `${BASE_IMAGE_URL}${invite.profileImageUrl}` : '/default-avatar.jpg'} 
                                      alt={invite.username} 
                                      className="w-full h-full object-cover"
                                    />
                                  </div>
                                ...
                                <div className="sm:ml-6 flex-1 text-center sm:text-left">
                                  <h3 className="font-semibold text-lg text-gray-900">{invite.username}</h3>
                                  <p className="text-sm text-gray-500 mt-1">
                                    <span className="inline-flex items-center">
                                      <Clock className="h-4 w-4 mr-1 text-amber-500" />
                                      Request sent: {formatDate(invite.requestedAt)}
                                    </span>
                                  </p>
                                </div>
                                <div className="mt-4 sm:mt-0">
                                  <button
                                    onClick={() => handleViewProfile(invite.userId)}
                                    className="px-6 py-3 bg-rose-600 hover:bg-rose-700 text-white rounded-lg transition-colors duration-200 font-medium"
                                  >
                                    View Profile
                                  </button>
                        ...
                        <div className="text-center py-16">
                          ...
                          <h3 className="text-xl font-medium text-gray-900">No sent invites</h3>
                          <p className="text-gray-500 mt-2">You haven't sent any connection requests yet</p>
                        </div>
                      )}
                    </>
                  )}
                    ...
                    <>
                      {getPaginatedItems(receivedInvites).length > 0 ? (
                            ...
                            >
                              <div className="flex flex-col items-center">
                                ...
                                <div className="flex flex-col sm:flex-row mt-5 gap-3 w-full justify-center">
                                  ...
                                  <button
                                    onClick={() => handleAcceptInvite(invite.connectionId)}
                                  ...
                                  <button
                                    onClick={() => handleRejectInvite(invite.connectionId)}
                        ...
                        <div className="text-center py-16">
                          ...
                          <h3 className="text-xl font-medium text-gray-900">No received invites</h3>
                  ...
                  {/* Pagination */}
                  {totalPages(activeTab === 'connections' ? connections : 
                             activeTab === 'sent-invites' ? sentInvites : receivedInvites) > 1 && (
                    <div className="flex justify-center mt-10">
                      <div className="flex flex-wrap gap-2 justify-center">
                        ...
                        {Array.from({ length: totalPages(activeTab === 'connections' ? connections : 
                                                    activeTab === 'sent-invites' ? sentInvites : receivedInvites) }, 
                                   (_, i) => i + 1).map(page => (
                          <button
                            key={page}
                        ...
                        <button
                          ...
                          disabled={currentPage === totalPages(activeTab === 'connections' ? connections : 
                                                          activeTab === 'sent-invites' ? sentInvites : receivedInvites)}
                          className={`px-4 py-2 rounded-md ${
                            currentPage === totalPages(activeTab === 'connections' ? connections : 
                                                  activeTab === 'sent-invites' ? sentInvites : receivedInvites) 
                              ? 'bg-gray-100 text-gray-400 cursor-not-allowed' 
                              : 'bg-gray-200 hover:bg-gray-300 text-gray-700'
                          }`}
