...
// setShareableUrl(`http://localhost:3000/invite?userId=${encodeURIComponent(encryptedUserId)}`);
    ...
    async function fetchMatches() {
      try {
        ...
        if (data.success) {
          ...
          extractFilterOptions(data.matches);
        } else {
          console.error('Failed to fetch matches:', data.message);
        }
      } catch (error) {
  ...
  const extractFilterOptions = (userData) => {
    const religions = Array.from(new Set(userData.map(user => user.religion).filter(Boolean)));
    const countries = Array.from(new Set(userData.map(user => user.country).filter(Boolean)));
    const cities = Array.from(new Set(userData.map(user => user.city).filter(Boolean)));
    const languages = Array.from(new Set(userData.flatMap(user => user.languages || []).filter(Boolean)));
  ...
  useEffect(() => {
    const applyFilters = () => {
      ...
      if (filters.religion !== 'all') {
        result = result.filter(user => user.religion === filters.religion);
      }
