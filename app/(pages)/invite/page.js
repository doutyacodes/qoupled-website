...
import { Heart, Loader2, CheckCircle2, AlertTriangle, ArrowRight } from 'lucide-react';
                ...
                try {
                    ...
                    if (!user) {
                        ...
                        if (sessionResp.hasUncheckedInvitation) {
                            ...
                            if (quizStatus == 'yes') {
                                setTimeout(() => {
                                    router.push(`/compatibility-check?userId=${encryptText(sessionResp.uncheckedInvitationId)}`);
                                }, 2000);
                            } else {
                                setTimeout(() => {
                                    router.push('/tests');
                            ...
                            if (quizStatus == 'yes') {
                                setTimeout(() => {
                                    router.push(`/compatibility-check?userId=${encryptText(inviteUserId)}`);
                                }, 1000);
                            } else {
                                setTimeout(() => {
                                    router.push(`/tests`);
        ...
        switch(status) {
            case "checking":
                return {
                    ...
                    message: "Please wait while we check your profile and compatibility status..."
                };
            case "redirecting":
                return {
            ...
            case "previous-session":
                return {
                    ...
                    message: "You have an incomplete compatibility check. We're taking you there now..."
                };
            case "error":
                return {
        ...
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-rose-400 to-red-500 p-4">
            <div className="bg-white rounded-2xl p-8 max-w-md w-full shadow-xl">
                ...
                <div className="w-full text-center my-8">
                    ...
                    <h2 className="text-2xl font-semibold text-gray-800 mb-2">{statusContent.title}</h2>
                ...
                {(status === "checking" || status === "redirecting") && (
                    <div className="flex justify-center mt-4 space-x-1">
                        <div className="w-2 h-2 bg-rose-400 rounded-full animate-pulse" style={{animationDelay: "0ms"}}></div>
                        <div className="w-2 h-2 bg-rose-400 rounded-full animate-pulse" style={{animationDelay: "300ms"}}></div>
                        <div className="w-2 h-2 bg-rose-400 rounded-full animate-pulse" style={{animationDelay: "600ms"}}></div>
                    </div>
                )}
            </div>
        </div>
